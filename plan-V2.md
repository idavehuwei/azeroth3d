...
| 阶段 | 状态 | 交付 / 备注 |
| --- | --- | --- |
| **R0** 色板 · MAT | ✅ | `palette.js` · commit `008b499` |
| **R1** 程序化纹理 | ✅ | `textures.js` · `2cbecc3` |
| **R2** 高度场地形 | ✅ | `terrain.js` · `a039ab9` |
| **R3** 植被 · 水体 · 道具 | ✅ | `props.js` · `7ab6c54` |
| **R4** 天空 · 光照 · 昼夜 | ✅ | `sky.js` · `671bbd7` |
| **R5** 人形骨架 · Anim | ✅ | `rig.js` · 骨架层级 + Anim 状态机 |
| **R6** 生物族群工厂 | ✅ | `creatures.js` 四足/人形怪/元素 + Boss 挥锤三段 |
| **R7** 战斗 VFX | ✅ | 四类模板 + 粒子池 + 受击/姓名板/Boss 刻度 |
| **R8** 性能预算 | ✅ | `debug.js` · 预算面板 · 移动阴影 1024 · 假 Bloom |
| **G1–G7** 玩法闭环 | ✅ G1–G7 全部收口 | 对照 plan-merged STEP 2–11/24；单机玩法闭环齐 |
| 附带 | ✅ | 小地图箭头对齐面向 + 本地视野丰富化 · `16e1934` |

**当前里程碑**：M1–M5 ✅（R0–R8 + G1–G7）· plan-V2 渲染/玩法主线收口。
...
| WoC 呈现清单 | 我们现在（2026-07-22） | 目标 | 对应步骤 |
| --- | --- | --- | --- |
| 绘入地形的道路、草丛、松树 + 橡树 | ✅ 高度场 + 顶点色土路 + InstancedMesh 草 + 松/橡变体 | 同左（已落地；振幅/密度可再调） | **R2 · R3** ✅ |
| 带动画水面的湖泊 | ✅ 镜湖 shader（波纹 / 菲涅尔 / 岸线） | 同左 | **R3** ✅ |
| 飘动的云朵、实时阴影 | ✅ 穹顶 shader + 云片 + 阴影相机跟随 ±35 | 同左 | **R4** ✅ |
| 木桁架房屋、教堂、集市摊位、帐篷、带闪烁灯光的营火 | 部分：tent/totem/campfire 已升级；木屋/摊位仍为 V1 工厂几何 | 继续打磨营地道具 | **R3** ✅ / 可迭代 |
| 十二个绑定骨骼的生物族类，配走 / 攻 / 施法 / 坐 / 死动画 | `buildBoar` / `buildFlameSpawn` 等仍硬编码，无统一族群骨架 | 3 个族群工厂（四足 / 人形 / 元素），共享动画状态机 | **R6** |
| 九职业共用程序化人形骨架，按职业换武器配色 | ✅ `rig.js` 真骨架 + `Anim`；职业 = `HUMANOIDS`/`CLASS_LOOK` 一条 | 保持；NPC 牛头人可再迁同一骨架 | **R5** ✅ |
| 为每个法术 / 物品 / 增益绘制的程序化图标 | ✅ `icons.js`（plan-v1 已做） | 保持 | G1（已有） |
| 浮动战斗文字、施法条、增益条、单位框架 | 已有 FCT / 血条 / buffs；VFX 注册表雏形 | 打磨：暴击放大、姓名板、AoE 预警 | **R7** |
| 程序化 WebAudio 音效 | ✅ `sfx.js`（plan-v1 已做） | 保持 | G5（已有） |

**一句话结论**：R0–R8 渲染线已收口（质感 → 世界 → 人物怪物 → VFX → 性能预算）；玩法缺口对照 G 表 / plan-v1。
...
### R1 · 程序化纹理库 `textures.js` `P0` · 1 次迭代 · ✅ 完成（2026-07-22）

这是**单位工作量收益最高的一步**。现在所有材质都是纯色 + 光照，塑料感的根源就在这。

**改动**

- 新建根目录 `textures.js`（与仓库布局一致，非 `js/`）：
  - 底层画笔：`valueNoise` / `streaks` / `speckle` / `cracks`（SeededRng，种子 `WORLD_SEED ^ hash("tex:"+key)`）
  - 11 条配方：`grass` `dirt` `rock` `bark` `leaf` `fur` `hide` `plate` `cloth` `bone` `magma`
  - 导出：`Tex.get` / `Tex.rough`（仅 rock/bark/plate）/ `Tex.normal`（仅 rock/bark）/ `Tex.bind` / `Tex.warm`
  - 分辨率 256（移动端 128），`RepeatWrapping`；预热后纹理对象数 ≤16
- `MAT.get` 挂钩：创建材质时按键自动 `Tex.bind` —— **未改任何 `build*`**
- `game.html`：`palette.js` 之后加载 `textures.js`
- 预览页：`tools/tex_preview.html`

**验收**

- [x] `tools/tex_preview.html` 网格铺满全部配方
- [x] 预热后 `Tex.size() ≤ 16`（单测）
- [x] 世界种子固定时贴图逐像素一致（单测）
- [ ] 浏览器内 `renderer.info.memory.textures ≤ 16`（需手动确认）

**风险**：低。纯渲染增强；共享贴图不 clone，避免爆内存。

---

### R2 · 地形升级：高度场 + 顶点着色 + 道路 `P0` · 1-2 次迭代 · ✅ 完成（2026-07-22）

**现状**：`grass = Mesh(CircleGeometry(WORLD_R+50, 64))`，全平、单色、无路。

**改动**

- 新建根目录 `terrain.js`：`PlaneGeometry((WORLD_R+50)*2, …, 180, 180)`，fbm 高度场（大≤3m / 中 1.2 / 细节 0.28，风险缓解保守振幅）。
- 导出 `heightAt(x,z)`：与顶点同一 hash-noise 公式；营地 / 双传送门平滑掩膜压平；湖盆微凹。
- 顶点着色：沙岸 / 陡坡露岩 / 草→土 / 道路混 dirt；可选叠 `Tex.get("grass")`。
- 道路：营地→熔火门二次贝塞尔，距线 < 3.5m 压平并染土色（去掉分段 Circle 土路 Mesh）。
- 接线：玩家 / 野怪 / NPC / 同伴 / 相机 / 掉落 / 采集 / 制作台 / `placeProp` / `enterZone` 莫高雷贴地；相机高度 lerp。

**验收**

- [x] `heightAt` 单测：同参同结果、营地压平、值域合理
- [x] 无大圆盘草皮；`game.html` 加载 `terrain.js`
- [ ] 浏览器目视：丘陵、露岩、土路、贴地脚（需手动）
- [x] `node test_step01.js` 含 R2 冒烟

**风险 · 中**：振幅先保守；仅 `mulgore` 贴地，副本/贫瘠/赭岩仍平坦。

---

### R3 · 植被 · 水体 · 场景道具 `props.js` `P0` · 1-2 次迭代 · ✅ 完成（2026-07-22）
...
- [x] `props.js` 工厂 + `spawnMulgoreProps` / `updateProps`
- [x] 替换旧散布树/岩/静态 pond；营地 tent/totem/campfire 升级
- [x] `BAL.props` + `game.html` / `main.js` 接线
- [x] `node test_step01.js` 含 R3 冒烟

---

### R4 · 天空 · 光照 · 阴影 · 昼夜 `P0` · 1 次迭代 · ✅ 完成（2026-07-22）
...
- [x] `sky.js` 穹顶 shader + `initZoneSky` / `updateSky`（render-only 铁律）
- [x] 阴影相机跟随玩家 ±35；补光；黎明/黄昏色
- [x] 副本岩浆光/雾/火星脉动；萤火虫 100
- [x] `node test_step01.js` 含 R4 冒烟

---

### R5 · 人形骨架 `rig.js` + 动画状态机 `P0` · 1-2 次迭代 · ✅ 完成（2026-07-22） 
...
- [x] `rig.js`：`createRigSkeleton` / `assembleHumanoidRig` / `Anim` / `updateHumanoidAnim`
- [x] `CLASS_LOOK` + `buildFromClassLook`；六职业走同一骨架；`handR` 挂武器
- [x] `main.js` / `companions.js` 改调 `updateHumanoidAnim`；受击 `animHitT`
- [x] `node test_step01.js` 含 R5 骨架/Anim 冒烟
...
- `creatures.js` 里 `MOB_LOOK`（= `QUADS`）配方表：加一种怪 = 加一条配置。

**验收**

- [x] 加「草原狼」= `MOB_LOOK.wolf` + `BALANCE.mobs.wolf` + `MOB_TYPES` 一行（工厂本体不动）
- [x] 四足对角迈步 + 死亡前腿软化侧倒；元素碎岩轨道 + 死亡碎散
- [x] Boss 挥锤蓄力→挥出→收势，对齐 `delayMs: 450`；熔核增亮 / 震屏 / 涟漪
- [x] `node test_step01.js` 全绿；`hitEntity` / `onDeath` 未改
...
- **死亡溶解**：`die` 动画结束后溶解；G1 已改为「有掉落则等拾取 / 尸体超时再溶解」。
- **粒子池化**：现在 `spawnBurst` 每次 new geometry 且不 dispose —— 改为固定容量对象池 + 复用几何体，顺手解决内存泄漏（plan-merged 里也列了这条）。

**验收**

- [x] `eruption_ring` / `venom_ring` 走 `ground_warn` 填充预警（对齐 `delay: 2.2`）
- [x] 粒子 `VFX_POOL` 复用几何；`tickVfx` 统一驱动
- [x] 受击闪白 0.12s + 后仰；FCT `.fct-crit`；姓名板距离/遮挡/精英描边；Boss 阶段刻度
- [x] 真言术：盾改菲涅尔 `holy_shield`；`node test_step01.js` 全绿
...
- ~~从 CDN 引入 three 的 `EffectComposer` + `UnrealBloomPass`~~（本刀未上 CDN examples，避免版本对齐风险）
- 退路落地：「假 bloom」外扩壳（`BackSide` + additive）——登录页齿轮 / 华丽档可开；CSS `#vignette` 保留。

**验收**

- [x] `debug.js`：`` ` `` / `?dev` 面板；FPS / Draw / Tris / Tex 对照 `BAL.perf`，超标告警
- [x] `cheat.tp / .level / .time / .seed`；`BAL.sky.shadowMapMobile:1024` + `refreshSunShadows`
- [x] 假 Bloom 开关进画面设置；`node test_step01.js` 全绿
...
| **G2** ✅ | 经验与等级（1→18，`BALANCE.levels`），升级金光 + 经验条 | 挂 `hitEntity` 的 `onDeath` | P1 |
...
- 既有：`icons.js` 配方工厂 · `items.js` QUALITY/ITEMS/LOOT · `dropLoot`/`tryLoot` · F → `tryInteract` 优先拾取 · 尸体灰化侧倒
- 本刀补齐：有掉落时 `awaitLoot`，溶解推迟到 **拾取完成** 或 **尸体超时**（`requestCorpseDissolve`）；无掉落则侧倒结束后溶解

**验收**

- [x] 杀怪 → 尸体倒地 + 地面掉落；靠近按 F 拾取，图标/品质色/日志齐全
- [x] 溶解不再抢在拾取前；`onLooted` / 尸体超时触发 `requestCorpseDissolve`
- [x] `node test_step01.js` 全绿

### G2 · 经验与等级 · ✅

- 既有：`BAL.levels`（max **18**，区域门需要）· `gainXP` 唯一入口 · `#pXp` 经验条 · 野怪/Boss/任务 `onDeath`→XP
- 本刀补齐：升级 `loot_spark` + `spawnBurst` 金光（`BAL.levels.levelUp`）；副本小怪 `BAL.*.xp` → `addDie`→`gainXP`

**验收**

- [x] 杀怪/交任务涨经验；升级 announce + SFX + 金光；经验条刷新
- [x] 野怪 / Boss / 副本小怪均经 `hitEntity`→`onDeath` 给经验
- [x] `node test_step01.js` 全绿

### G3 · 背包 / 装备 / 真换装 · ✅

- 既有：`B` 背包（36 格）· `C` 纸娃娃 10 槽 · `applyEquipStats`（`dmgMul` / `hpMax`）· `equipItem`/`unequipItem`
- R5 挂点：`assembleHumanoidRig` 武器挂 `handR`/`handL`；`setWeapon` 移除 `userData.weapon` 子组并装新武器（含萨弗拉斯）
- 现设计说明：装备栏在 **C 角色面板**（非 B 内嵌两槽）；背包格数 36（STEP 4 原文 16 已演进）

**验收**

- [x] 拾取 → B 可见 → 左键装备 → `S.p.dmgMul`/`hpMax` 变化
- [x] 主手装备调用 `setWeapon(player, model)`，挂点为 `rig.handR`（弓手 `handL`）
- [x] C 面板可卸下并还原默认武器；`node test_step01.js` 全绿

### G4 · 野怪 AI / 新怪 / 稀有 · ✅

- 社群仇恨：`aggroMob` + `stats.socialR` / `group`（狼群）
- 脱战回巢：`leashR` → `return` + `BAL.leash.regenPct` 回血；回巢中不可打
- 三种新怪：陆行鸟（被动）· 草原狼（群）· 鹰身女妖（读条）；稀有「老灰鬃」+ `rares.js`

**验收**

- [x] 打狼一只全群上；陆行鸟不主动；稀有金色名 + 长重生
- [x] `node test_step01.js` 全绿

### G5 · 程序化音效 · ✅

- `sfx.js`：`SOUNDS` 参数表 + WebAudio 合成；仇恨统一 `growl`；`MOB_HIT` / `playHit` 按怪种
- 启程 `SFX.init()`；草原/副本 `MUSIC` 切换

**验收**

- [x] 调音只改数据表；换怪不加新音效代码路径
- [x] `node test_step01.js` 全绿

### G6 · 副本工厂 + 熔岩巨兽 · ✅

- `raid.js`：`DUNGEON` 分段 corridor→boss1→bridge→boss；`createBoss` / `defineBoss` 数据驱动
- 第二 Boss：`magmadar` 显示名「熔岩巨兽 · 玛格曼达」，模型 `QUADS.lavabeast`（对齐 plan-V2 字面）
- 终 Boss 拉戈斯；多副本 `DUNGEONS`（哀嚎/怒焰/奥妮）

**验收**

- [x] 清走廊 → 熔岩巨兽 → 岩桥 → 拉戈斯；无新粒子循环引擎分支
- [x] `node test_step01.js` 全绿

### G7 · 天赋 + 单机存档 · ✅

- `talents.js`：六职业双枝三层；`N` 键面板；`BAL.talents` 数值
- `save.js`：纯数据 localStorage + Base64 导出/导入；天赋/装备/等级重建

**验收**

- [x] 加点改 CD/数值；导出码可导入；刷新后进度恢复
- [x] `node test_step01.js` 全绿
...
| 里程碑 | 步骤 | 交付物 | 状态 |
| --- | --- | --- | --- |
| **M1 · 有质感** | R0 + R1 | 色板统一、程序化贴图挂满全场景 | ✅ 2026-07-22 |
| **M2 · 会动了** | R5 + R6 | 人形骨架 + 动画状态机、3 个生物族群、Boss rig 化 | ✅ |
| **M3 · 有世界** | R2 + R3 | 高度场地形 + 顶点着色 + 道路、草 / 树 / 岩 / 湖 / 云 | ✅ 2026-07-22 |
| **M4 · 有氛围** | R4 + R7 + R8 | 天空穹顶、跟随阴影、昼夜、VFX 注册表、性能达标 | ✅ 2026-07-22 |
| **M5 · 有玩法** | G1-G7 | 掉落 / 等级 / 背包 / 新怪 / 音效 / 副本 / 天赋 / 存档 | ✅ 2026-07-22 |

实际执行顺序与上表「推荐顺序」不同：先做了 **M1 → M3 → R4**，再回头做 **M2（R5/R6）**，随后 R7/R8 与 G1–G7 收口——plan-V2 主线已齐。
...
*MOLTEN CORE PROJECT · PLAN V2（单机 · 渲染优先）· 进度更新 2026-07-22*
*参考：levy-street/world-of-claudecraft README.zh_CN「呈现」与「Play offline」两节（代码 MIT / 少量素材 CC0）*
*基线：v1.3.5 + STEP 0 / STEP 1；plan-V2 已完成 R0–R8 + G1–G7；里程碑 M1–M5 收口*
*附带：小地图面向箭头 + 本地视野（`16e1934`）*
