...
> 基线 v1.3.5（五模块）· 目标：按 13 个可验证小步走完 v1.4 → v3.0 · 参考仓库 levy-street/world-of-claudecraft（代码 MIT / 少量资源 CC0）
...
### STEP 12 · AI 队友（伪组队）`P2`

- 招募 1 名 AI 队友（复用人形基座与职业技能）：跟随 / 集火你的目标 / 血量低于阈值时治疗。WoC 的 delve 模式里 AI 同伴 Tessa 就是这个形态——先做一个同伴，比直接做组队便宜一个数量级，且立刻有「开荒团」味道。

### STEP 13 · 新地图 · 牧师 · 真联机 `P3`

- **贫瘠之地**：现有 `sceneWorld / sceneRaid` 双场景机制推广为多场景注册表；地形复用 SeededRng，换种子即换地图。
- **牧师**：`CLASSES` 加一项 + 一条人形基座配置（治疗链 / 神圣新星 / 吸收盾——吸收盾机制 WoC 已验证：先扣盾再扣血，一个字段搞定）。
...
| v1.5 世界的呼吸 | 5-7 | 族群工厂 + 新怪×3 + 稀有精英、sfx.js、昼夜 | ⏳ 待开始 | 2-3 次迭代 |
| v2.0 火焰之地 | 8-11 | raid.js 分段、Boss 工厂 + VFX 注册表 + 熔岩巨兽、天赋、存档 | ⏳ 待开始 | 3-4 次迭代 |
| v3.0 远景 | 12-13 | AI 队友 → 新地图 / 牧师 / 联机 | 🔮 待定 | 开放式 |

**下一步行动**：从 **STEP 0** 开始（BALANCE 表 + SeededRng，约半天工作量、零玩法风险），完成后立即进入 STEP 2 掉落系统拿到第一个强反馈。
