...
- 死亡不再直接 `game over` 锁死：释放灵魂 → 营地灵魂医者旁复活（血量/怒气重置，装备耐久暂不做）。
- 副本内死亡：回走廊入口或退出副本确认框（经典「跑尸」的简化版：传送回复活点）。
- `BALANCE.death`：复活虚弱 10s（移速 -30%，可选）。

> **WoC 对照**：graveyards + release spirit；失败可恢复，鼓励再战。

> **验收**：野外死亡 → 复活 → 继续任务；副本团灭可重来且进度（走廊已清）按设计保留或重置（写进 `DUNGEON` 策略表）。
